  ...
  <v-container min-height="600px" :loading="contentLoading">
    ...
    >
      ...
      >
        ...
        <v-row v-if="question.type === 4 && !exam.answerList[index].score">
          <v-col cols="12" sm="6" md="3">
            <v-text-field
              outlined
              v-model="exam.answerList[index].givenScore"
              prepend-inner-icon="mdi-lightbulb-on-outline"
              :label="'输入分值(满分:' + question.score + ')'"
              required
            ></v-text-field>
          </v-col>
          ...
          <v-col cols="12" sm="6" md="3">
            <v-btn
              ...
              @click="
                gradeAnswer(
                  exam.answerList[index].id,
                  exam.answerList[index].givenScore,
                  index
                )
              "
            >
...
export default {
    ...
    return {
      breadcrumbs: [
        ...
        },
        {
          ...
          disabled: false,
          exact: true,
          to: "/admin/paper/result?paperId=" + this.$route.query.paperId,
        },
        {
          text: "学生答卷打分",
          disabled: true,
  ...
  methods: {
    ...
    getExamResult(paperId, writerId) {
      this.contentLoading = true;
      this.$api
        .getPaperResult({ paperId: paperId, userId: writerId })
        .then((res) => {
    ...
    gradeAnswer(answerId, score, index) {
      if (score > this.exam.questionList[index].score) {
        this.$toast({
          color: "error",
          mode: "",
          snackbar: true,
      ...
      this.$api
        .gradeAnswer({ answerId: answerId, score: score })
        .then((res) => {
          if (res.data.code == 200) {
            this.$set(this.exam.answerList[index], "score", score);
            this.$toast({
              color: "success",
              mode: "",
              snackbar: true,
  ...
  mounted() {
    this.getExamResult(this.$route.query.paperId, this.$route.query.writerId);
  },
};
</script>
