...
export default {
  ...
  methods: {
      ...
      this.$api
        ...
        .then((res) => {
          if (res.data.code == 200) {
            ...
            for (const question of this.test.questionList) {
            ...
            if (
              this.isEffectiveDate(this.test.startTime, this.test.endTime) == 0
            ) {
              this.startTimeDown();
            } else {
              this.$toast({
    ...
    handIn() {
      ...
      var questionListClone = JSON.parse(
        JSON.stringify(this.test.questionList)
      );
    ...
    timeDown() {
      const endTime = new Date(this.test.endTime);
  ...
  mounted() {
    this.getEest(this.$route.params.id);
  },
};
</script>
