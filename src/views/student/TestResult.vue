<template>
  <v-container min-height="600px" :loading="contentLoading">
    ...
    <v-card
      ...
      flat
      v-for="(question, index) in test.questionList"
      :key="index"
      elevation="2"
    >
      <v-card-title>
        ...
        <span v-if="!(question.type === 4 && !question.score)">
          <v-chip
            class="text-button"
            small
            v-if="test.answerList[index].score == question.score"
            color="success"
          >
          ...
          <v-chip
            class="text-button"
            small
            v-else-if="test.answerList[index].score == 0"
            color="error"
          >
          ...
          <v-chip
            class="text-button"
            small
            v-else-if="test.answerList[index].score < question.score"
            color="warning"
          >
            <v-icon class="mr-2">mdi-alert</v-icon
            >{{ test.answerList[index].score + "/" + question.score }}
          </v-chip>
        </span>
      </v-card-title>
      ...
      <v-card-actions class="px-6">
        <v-row class="px-2" v-if="question.type === 0">
          <v-radio-group
            v-model="test.answerList[index].answer"
            label="你的答案"
            row
            readonly
          >
        ...
        <div class="px-2" v-if="question.type === 3">
          ...
          >
          <v-row
            v-for="(answer, answerIndex) in test.answerList[index].answer"
            :key="answerIndex"
          >
            <v-text-field
              v-model="test.answerList[index].answer[answerIndex]"
              clearable
              readonly
              outlined
      ...
      >
        ...
        <v-alert
          ...
          dismissible
          type="info"
          v-if="question.type === 4 && !test.answerList[index].score"
        >
          等待老师批改!
        </v-alert>
      </v-card-actions>
...
export default {
      ...
      this.$api
        ...
        .then((res) => {
          if (res.data.code == 200) {
            ...
            for (
              let index = 0;
              index < this.test.questionList.length;
              index++
            ) {
              let question = this.test.questionList[index];
              let answer = this.test.answerList[index];
  ...
  mounted() {
    this.getTestResult(this.$route.params.id);
  },
};
</script>
